...
type AccessInfo = {
  ...
  info: {
    exam: {
      ...
      maxFullscreenExits?: number; snapshotIntervalSec?: number;
    };
...
export default function ExamPrep() {
  ...
  // إقرار قراءة التعليمات (طلب الإدارة) — شرط واجهة قبل البدء
  ...
  const canStart = consentPolicy && consentProctoring && readInstructions && allChecks;
      ...
      <div className="mx-auto max-w-3xl space-y-5 px-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            {/* تعليمات الاختبار الخاصة (إن كتبها المشرف) */}
            {(exam.instructionsAr || exam.instructionsEn) && (
            ...
            {/* القواعد الأساسية بأرقام هذا الاختبار + رابط الدليل الكامل (طلب الإدارة) */}
            <div className="rounded-lg border-2 border-amber-300 bg-amber-50 p-4">
              <div className="mb-2 flex flex-wrap items-center justify-between gap-2">
                <h3 className="flex items-center gap-2 font-bold text-amber-900"><AlertTriangle className="h-4 w-4" /> {t("keyRules")}</h3>
                <Link to="/exam/guide" target="_blank" rel="noreferrer" className="flex items-center gap-1 text-sm font-medium text-brand underline">
                  <BookOpen className="h-4 w-4" /> {t("readFullGuide")}
                </Link>
              </div>
              <ol className="list-decimal space-y-1.5 ps-5 text-sm leading-relaxed text-amber-950">
                {quickRules.map((r, i) => <li key={i}>{fillVars(lang === "ar" ? r.ar : r.en, vars)}</li>)}
              </ol>
            </div>
            ...
            <div className="space-y-3 rounded-lg border p-4">
              ...
              <label className="flex items-start gap-3 text-sm">
                <Checkbox checked={readInstructions} onCheckedChange={(v) => setReadInstructions(!!v)} className="mt-0.5" />
                {t("iReadInstructions")}
              </label>
            </div>
