...
export default function Exams() {
  ...
  const create = trpc.exam.create.useMutation({
    onSuccess: () => { utils.exam.list.invalidate(); setOpen(false); toast.success(t("toastAdded")); },
    onError: (e) => toast.error(e.message),
  });
  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({
    titleAr: "", titleEn: "", durationMin: 360, passScore: 60, // المدة الافتراضية 360 دقيقة (ملاحظة 4)
    shuffleQuestions: true, shuffleOptions: true, lockdown: true, proctoring: true, cameraRequired: false,
    snapshotIntervalSec: 60, maxFullscreenExits: 3,
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  <Input type="number" min={1} max={600} value={form.durationMin} onChange={(e) => setForm({ ...form, durationMin: +e.target.value })} />
                </div>
                <div className="grid gap-2">
              ...
              {/* إعدادات المراقبة والأمان (المتطلب 5/8) */}
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label>{t("snapshotInterval")}</Label>
                  <Input type="number" min={10} max={600} value={form.snapshotIntervalSec} onChange={(e) => setForm({ ...form, snapshotIntervalSec: +e.target.value })} />
                </div>
                <div className="grid gap-2">
                  <Label>{t("maxFullscreenExits")}</Label>
                  <Input type="number" min={0} max={20} value={form.maxFullscreenExits} onChange={(e) => setForm({ ...form, maxFullscreenExits: +e.target.value })} />
                </div>
              </div>
