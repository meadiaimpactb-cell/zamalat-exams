  ...
  return (
    <div className="flex min-h-screen items-center justify-center brand-gradient p-4">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-2xl">
        ...
        <p className="mt-3 text-muted-foreground">{FEATURES.candidateResultLookup ? t("resultLater") : t("resultOnMainSite")}</p>
        {FEATURES.candidateResultLookup ? (
          <Link to="/exam"><Button className="mt-6 bg-brand">{t("checkResult")}</Button></Link>
        ) : (
          <Link to="/"><Button className="mt-6 bg-brand">{t("back")}</Button></Link>
        )}
      </div>
    </div>
  );
