...
export const users = mysqlTable(
  "users",
  {
    ...
    role: mysqlEnum("role", ["super_admin", "admin", "tech_general", "fellowship_manager", "expert"]).notNull(),
...
// ربط مدير الزمالة بالزمالات التي يديرها (نطاق الرؤية)
export const userFellowships = mysqlTable(
  "user_fellowships",
  {
    id: serial("id").primaryKey(),
    userId: bigint("userId", { mode: "number", unsigned: true }).notNull(),
    fellowshipId: bigint("fellowshipId", { mode: "number", unsigned: true }).notNull(),
    createdAt: timestamp("createdAt").notNull().defaultNow(),
  },
  (t) => [uniqueIndex("user_fellowship_uq").on(t.userId, t.fellowshipId)],
);
...
export const fellowships = mysqlTable("fellowships", {
  id: serial("id").primaryKey(),
  nameAr: varchar("nameAr", { length: 255 }).notNull(),
  nameEn: varchar("nameEn", { length: 255 }), // اختياري (ملاحظة 9)
  description: text("description"),
  createdAt: timestamp("createdAt").notNull().defaultNow(),
});

/* ---------------------------------- Tracks & Cohorts (تابعة للزمالة) ---------------------------------- */
// المسارات: كل مسار يتبع زمالة معينة
export const tracks = mysqlTable(
  ...
  (t) => [index("tracks_fellowship_idx").on(t.fellowshipId)],
);

// الدفعات: كل دفعة تتبع زمالة معينة
export const cohorts = mysqlTable(
  ...
  (t) => [index("cohorts_fellowship_idx").on(t.fellowshipId)],
);
...
export const questions = mysqlTable(
  "questions",
  {
    ...
    textEn: text("textEn"),
    // صورة/مخطط داخل نص السؤال — data URL بصيغة base64 (المتطلب 1)
    imageUrl: mediumtext("imageUrl"),
    // هل يُسمح للمشارك بإرفاق صورة كإجابة (للأسئلة المقالية/القصيرة) — المتطلب 1
    allowImageAnswer: boolean("allowImageAnswer").notNull().default(false),
...
export const exams = mysqlTable(
  "exams",
  {
    ...
    // المدة الافتراضية 360 دقيقة (ملاحظة 4) — قابلة للتعديل لكل اختبار
    durationMin: int("durationMin").notNull().default(360),
    ...
    // كل كم ثانية تُلتقط لقطة كاميرا أثناء الاختبار (المتطلب 5)
    snapshotIntervalSec: int("snapshotIntervalSec").notNull().default(60),
    // عدد مرات الخروج من ملء الشاشة المسموح بها قبل الإنهاء التلقائي (0 = بلا حد) — المتطلب 8/10
    maxFullscreenExits: int("maxFullscreenExits").notNull().default(3),
...
export const examSessions = mysqlTable(
  "exam_sessions",
  {
    ...
    // إعادة فتح جلسة مقفَلة مع الاحتفاظ بالإجابات (ملاحظة 6) — عدّاد الخروج من ملء الشاشة يُحسب بعد آخر إعادة فتح
...
export const answers = mysqlTable(
  "answers",
  {
    ...
    // إجابة على شكل صورة/مخطط يرفعها المشارك — data URL base64 (المتطلب 1)
    answerImageUrl: mediumtext("answerImageUrl"),
...
// لقطات الكاميرا الدورية أثناء الاختبار (المتطلب 5)
export const sessionSnapshots = mysqlTable(
  "session_snapshots",
  {
    id: serial("id").primaryKey(),
    sessionId: bigint("sessionId", { mode: "number", unsigned: true }).notNull(),
    image: longtext("image").notNull(), // data URL base64 (JPEG منخفض الدقة)
    kind: mysqlEnum("kind", ["camera", "screen"]).notNull().default("camera"),
    flagged: boolean("flagged").notNull().default(false),
    capturedAt: timestamp("capturedAt").notNull().defaultNow(),
  },
  (t) => [index("session_snapshots_session_idx").on(t.sessionId)],
);
