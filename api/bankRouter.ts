...
import { questionBanks, questions } from "@db/schema";
...
export const bankRouter = createRouter({
  listBanks: staffProcedure.query(async () => {
    return getDb().select().from(questionBanks).orderBy(questionBanks.id);
  }),

  createBank: staffProcedure
    .input(z.object({ nameAr: z.string().min(2), nameEn: z.string().min(2), fellowshipId: z.number().optional() }))
    .mutation(async ({ ctx, input }) => {
      const [{ id }] = await getDb().insert(questionBanks).values({
        nameAr: input.nameAr, nameEn: input.nameEn, fellowshipId: input.fellowshipId ?? null,
      }).$returningId();
