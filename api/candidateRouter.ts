...
import {
  ...
  questions, answers, answerScores, results, sessionEvents,
} from "@db/schema";
...
export const candidateRouter = createRouter({
  ...
  state: publicQuery
    ...
    .query(async ({ input }) => {
      const { session, exam, cand } = await getSession(input.token);
      ...
      return {
        ...
        questions: out,
        answers: saved.map((a) => ({ examQuestionId: a.examQuestionId, answer: a.answer, answerText: a.answerText, flagged: a.flagged })),
  ...
  logEvent: publicQuery
    ...
    .mutation(async ({ input }) => {
      ...
      const riskAdd = { info: 0, low: 2, medium: 6, high: 15, critical: 30 }[input.severity];
      if (riskAdd > 0) {
